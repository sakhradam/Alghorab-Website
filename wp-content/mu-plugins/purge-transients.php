...
Description: Purge automatique des transients WP.
Author:       Seebz
*/
if ( ! function_exists('wps_purge_transients') ) {
	function wps_purge_transients($older_than = '7 days', $safemode = true) {
		global $wpdb;
...
function wps_purge_transients_activation () {
	if (!wp_next_scheduled('purge_transients_cron')) {		
		wp_schedule_event( time(), 'daily', 'purge_transients_cron');
	}
}
register_activation_hook(__FILE__, 'wps_purge_transients_activation');



function my_plugin_deactivation_wps () {
	if (wp_next_scheduled('purge_transients_cron')) {
...
function wps_do_purge_transients_cron () {
	purge_transients();
}
add_action('purge_transients_cron', 'wps_do_purge_transients_cron');